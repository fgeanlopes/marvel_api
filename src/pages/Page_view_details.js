...
import "./scss/Page_view_details.scss";

class Page_view_details extends Component {
    constructor(props) {
        super(props);
        this.state = {
            series_status: false,
    ...
    receivedData() {
        ...
        if (checker.path === "/comic/details/:id") {
            const searchComic = `comics/${idSearch}?${KeyMarvel}`;
            ...
            const searchCharacter = `characters/${idSearch}?${KeyMarvel}`;
            //FIELDS REQUEST
            const status = true;
            this.setState({
    ...
    render() {
        // Validação de state para imformação de personagem.
        ...
        // Validação de state para imformação de Quadrinho.
...
export default Page_view_details;